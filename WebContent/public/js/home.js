...
function checkSentDatePost() {
    ...
    jsonRequestBean["sentFlag"] = sentFlag;
    jsonRequestBean["fruitBoxList"] = fruitBoxList;
...
function renderFruitTable() {
    // 最初の行を除くすべての表の行を削除する
     $("#fruitTable").find("tr:gt(1)").remove();
    // var tempTr = $("#fruitTable tr:first").clone().end();
    // $("#fruitTable").find("tr:gt(0)").remove();
    var tempTr = $("#fruitTable tr:nth-child(2)").clone().end();
    $.each(objFruitBoxList, function(i, row) {
